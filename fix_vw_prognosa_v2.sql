-- ================================================
-- FIX PROGNOSA VIEW - Versi Sederhana Tanpa Collation Issue
-- ================================================

-- Step 1: Drop view yang rusak
DROP VIEW IF EXISTS vw_prognosa;

-- Step 2: Recreate view dengan struktur sederhana
-- Menggunakan kolom langsung tanpa CAST untuk menghindari collation error
CREATE VIEW vw_prognosa AS
SELECT 
    jenis_anggaran,
    CONCAT(COALESCE(nomor_prk, ''), ' - ', COALESCE(uraian_prk, '')) AS rekap,
    
    -- Prognosa per bulan (dari renc_byr_bln)
